-- First, drop dependent functions and policies, then recreate enum
...
DROP POLICY IF EXISTS "Admins and moderators can view contact submissions" ON public.contact_submissions;
DROP POLICY IF EXISTS "Admins can update contact submissions" ON public.contact_submissions;
DROP POLICY IF EXISTS "Admins can delete contact submissions" ON public.contact_submissions;

-- Now we can safely drop and recreate the enum
DROP TYPE app_role CASCADE;
CREATE TYPE app_role AS ENUM ('admin', 'moderator', 'user', 'student', 'school');
...
FOR SELECT 
USING (current_user_has_role('admin'::app_role) OR current_user_has_role('moderator'::app_role));
